import { defineGkdApp } from '@gkd-kit/define';

export default defineGkdApp({
  id: 'com.cainiao.wireless',
  name: '菜鸟',
  groups: [
    {
      key: 0,
      name: 'in-app|OYog11xiTVhaKaeJrWRlAFhSqCFMl1L7GaRkP2QcE44=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_IT"][clickable=true] > android.view.View[id="_IS"][clickable=true] > android.view.View[id="_IR"][clickable=true] > android.view.View[id="_IQ"][clickable=true] > android.view.View[id="_Ip"][clickable=true] > android.view.View[id="_Io"][clickable=true] > android.view.View[id="_Im"][clickable=true] > android.view.View[id="_Il"][clickable=true] > android.view.View[id="_Ik"][text=""][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 1,
      name: 'in-app|b5lGSR2nIUSUv9jjCu8KcWSSAUo2-O2uEYXNhugLJQQ=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Jq"][clickable=true] > android.view.View[id="_Jp"][clickable=true] > android.view.View[id="_Jo"][clickable=true] > android.view.View[id="_Jn"][clickable=true] > android.view.View[id="_Lr"][clickable=true] > android.view.View[id="_Lq"][clickable=true] > android.view.View[id="_Lo"][clickable=true] > android.view.View[id="_Ln"][clickable=true] > android.view.View[id="_Lm"][clickable=true] > android.view.View[id="_Ll"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 2,
      name: 'system|1KqJ9fkZ5_PlFMD0XdR0VhOOxE-zXj2B1nXFuLv9-F4=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.ScrollView[clickable=true] > android.widget.LinearLayout[id="com.android.permissioncontroller:id/grant_singleton"][clickable=true] > android.widget.LinearLayout[id="com.android.permissioncontroller:id/grant_dialog"][clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.Button[id="com.android.permissioncontroller:id/permission_allow_button"][text="允许"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 3,
      name: 'in-app|5cH0Hv70ZqlIpnEE1LGp_1_b1k7MoTFuZTW2hb6CL0o=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Ao"][clickable=true] > android.view.View[id="_An"][clickable=true] > android.view.View[id="_Am"][clickable=true] > android.view.View[id="_BY"][clickable=true] > android.view.View[id="_BX"][clickable=true] > android.view.View[id="_BW"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 4,
      name: 'in-app|VysmszcFBlvnnfPxn9lv2h0EN2Ew7DAuwaPJhoiHW04=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/close_layout"][clickable=true] > android.widget.ImageView[id="com.cainiao.wireless:id/gg_dialog_base_close"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 5,
      name: 'in-app|lj2jGM8e3_P7KKm0aKSVfkjvwwtDfnaR6vsB6XRMEvs=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '.cubex.mvvm.view.CubeXJSActivity',
          matches: 'android.widget.FrameLayout[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 6,
      name: 'in-app|X3oJklpqyEavsplNNlulO_0CqaY1ahYhGh2GstjDJxk=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.taobao.cainiao.logistic.ui.view.LogisticDetailActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/layout_logistic_detail"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/logistic_detail_activity_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/layout_frame"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nested_scrolling_parent2_layout"][clickable=true] > android.support.v7.widget.RecyclerView[id="com.cainiao.wireless:id/logistic_detail_recycleview"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/layout_root"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/layout_content"][clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/layout_root"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.view.View[desc="复制"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 7,
      name: 'in-app|ESi2AkXL_ItE0RnH3AkkFTjP3rDJ-w6g8aK3U2jMbYc=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_QI"][clickable=true] > android.view.View[id="_QH"][clickable=true] > android.view.View[id="_QG"][clickable=true] > android.view.View[id="_QF"][clickable=true] > android.view.View[id="_QB"][clickable=true] > android.view.View[id="_QA"][clickable=true] > android.view.View[id="_Pz"][clickable=true] > android.view.View[id="_QY"][clickable=true] > android.view.View[id="_Qq"][clickable=true] > android.view.View[id="_Qp"][clickable=true] > android.view.View[id="_Qo"][clickable=true] > android.view.View[id="_Qn"][clickable=true] > android.view.View[id="_Qm"][clickable=true] > android.view.View[id="_UX"][clickable=true] > android.view.View[id="_UW"][text=""][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 8,
      name: 'in-app|XhApULYmoU-e0rmUDi9yoKBhxPrhDBwYEP2JSv5QyLY=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.alipay.mobile.nebulax.integration.mpaas.activity.NebulaActivity$Main',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Jo"][clickable=true] > android.view.View[id="_Jn"][clickable=true] > android.view.View[id="_Jm"][clickable=true] > android.view.View[id="_Jl"][clickable=true] > android.view.View[id="_LZ"][clickable=true] > android.view.View[id="_LY"][clickable=true] > android.view.View[id="_LX"][clickable=true] > android.view.View[id="_ADn"][clickable=true] > android.view.View[id="_ADm"][clickable=true] > android.view.View[id="_ADl"][clickable=true] > android.view.View[id="_ADk"][clickable=true] > android.view.View[text="我已阅读并同意，下次不再提示"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 9,
      name: 'in-app|h1lcIt6Q4dNmfB-TzPEbMRInbXiXZ-6HON7_-mFWZgg=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_OI"][clickable=true] > android.view.View[id="_OH"][clickable=true] > android.view.View[id="_OG"][clickable=true] > android.view.View[id="_OF"][clickable=true] > android.view.View[id="_Oe"][clickable=true] > android.view.View[id="_Od"][clickable=true] > android.view.View[id="_Ob"][clickable=true] > android.view.View[id="_Oa"][clickable=true] > android.view.View[id="_OZ"][text=""][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 10,
      name: 'in-app|9uhPoLXaulKF-HVl3tQuB0AiePn5TYr2mX_I19IADU8=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/browser_fragment_layout"][clickable=true] > com.uc.webview.export.WebView[desc="WVUCWebView"][clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[text="菜鸟回收"][clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.widget.Button[text="关闭"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 11,
      name: 'in-app|8eLfQdV6n_MAPRW543o1OFYCFJpssKg7OQRQRXAlQEg=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/browser_fragment_layout"][clickable=true] > com.uc.webview.export.WebView[desc="WVUCWebView"][clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[text="红包乐园"][clickable=true] > android.view.View[id="root"][clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 12,
      name: 'in-app|PU31b9M2Lw3Z_yUJsJWF27hKlyRPLWCTAN08w5W0d7c=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.alipay.mobile.nebulax.integration.mpaas.activity.NebulaActivity$Main',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Ur"][clickable=true] > android.view.View[id="_Uq"][clickable=true] > android.view.View[id="_Up"][clickable=true] > android.view.View[id="_Uo"][clickable=true] > android.view.View[id="_UQ"][clickable=true] > android.view.View[id="_UP"][clickable=true] > android.view.View[id="_UO"][clickable=true] > android.view.View[id="_UN"][clickable=true] > android.view.View[id="_Yn"][clickable=true] > android.view.View[id="_Ym"][clickable=true] > android.view.View[id="_Yl"][clickable=true] > android.view.View[text="确定"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 13,
      name: 'in-app|FpXTH8U4pmNDpCTgNO0YXloIywP6IrzRKyzQ4BrwQ0U=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Ao"][clickable=true] > android.view.View[id="_An"][clickable=true] > android.view.View[id="_Am"][clickable=true] > android.view.View[id="_BY"][clickable=true] > android.view.View[id="_BX"][clickable=true] > android.view.View[id="_BW"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 14,
      name: 'in-app|Bk9psfZgN-XICm7EiboNJYen1MraEVBhA8Mo5EgERmo=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.alipay.mobile.nebulax.integration.mpaas.activity.NebulaActivity$Main',
          matches: 'android.widget.FrameLayout[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 15,
      name: 'in-app|0Vb5lGBaQkb-HWRwRiRmTCbalDSmypebI5gLkwsdeNM=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Ao"][clickable=true] > android.view.View[id="_An"][clickable=true] > android.view.View[id="_Am"][clickable=true] > android.view.View[id="_BV"][clickable=true] > android.view.View[id="_BU"][clickable=true] > android.view.View[id="_BT"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 16,
      name: 'in-app|ppeOiOqPNuCZrg83AGIAgJyZIMQ8_h2c3QBhbGEdN4U=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.android.systemui:id/status_bar_container"][clickable=true] > android.widget.FrameLayout[id="com.android.systemui:id/status_bar"][clickable=true] > android.widget.LinearLayout[id="com.android.systemui:id/status_bar_contents"][clickable=true] > android.widget.LinearLayout[id="com.android.systemui:id/phone_status_bar_left_container"][clickable=true] > android.widget.TextView[id="com.android.systemui:id/clock"][text="凌晨1:42"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 17,
      name: 'system|oa4GQXWAFxRQjI02saQUn6erSPDTpnhrTD1HNiu5q7Y=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.android.packageinstaller.permission.ui.GrantPermissionsActivity',
          matches: 'android.widget.FrameLayout[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 18,
      name: 'in-app|DZ_SuI3iJAPUwz1oH4D3koBet4R9VunKTImHeBieqdE=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.alipay.mobile.nebulax.integration.mpaas.activity.NebulaActivity$Main',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_APM"][clickable=true] > android.view.View[id="_APL"][clickable=true] > android.view.View[id="_APK"][clickable=true] > android.view.View[id="_APJ"][clickable=true] > android.view.View[id="_API"][clickable=true] > android.view.View[id="_APH"][clickable=true] > android.view.View[id="_APG"][clickable=true] > android.view.View[text="0元认证领卡"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 19,
      name: 'in-app|NC7wUOd1LIh4VtWx80NdbT_LO-ys3YeTGfkvIywvumE=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/gg_dialog_base_body"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/gg_dialog_base_button"][clickable=true] > android.widget.Button[text="马上开启"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 20,
      name: 'in-app|EmPAgAmBaAKBQwS3ADYoOUwXZ2xcTZ3IOmCNSyILgSA=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/gg_dialog_base_body"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/gg_dialog_base_button"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 21,
      name: 'in-app|8Naw98H62ofJOGN93kHyF9181eV1WupH3zegUTFZcHo=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.taobao.cainiao.logistic.ui.view.LogisticDetailActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/container"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/coordinator"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/design_bottom_sheet"][clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/cardview_share"][clickable=true] > android.widget.RelativeLayout[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 22,
      name: 'in-app|QPfZePu0G5lVZqfCJz7Kw7p9K5YidF9cF7tytOZJTVw=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/browser_fragment_layout"][clickable=true] > com.uc.webview.export.WebView[desc="WVUCWebView"][clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[text="幸运包裹"][clickable=true] > android.view.View[id="root"][clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[text="攻略"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 23,
      name: 'in-app|SEN9OFYhB5I93kyf6544RwVKPK8RA3PpUVEzbhNHjNU=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Ks"][clickable=true] > android.view.View[id="_Kr"][clickable=true] > android.view.View[id="_Kq"][clickable=true] > android.view.View[id="_Kp"][clickable=true] > android.view.View[id="_KH"][clickable=true] > android.view.View[id="_KG"][clickable=true] > android.view.View[id="_KF"][clickable=true] > android.view.View[id="_KB"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 24,
      name: 'in-app|KgDzkQSfbYqiHAOsU9SjoliGEBhOU4x2OAdwGBNIXXc=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.android.systemui:id/status_bar_container"][clickable=true] > android.widget.FrameLayout[id="com.android.systemui:id/status_bar"][clickable=true] > android.widget.LinearLayout[id="com.android.systemui:id/status_bar_contents"][clickable=true] > android.view.ViewGroup[id="com.android.systemui:id/system_icon_area"][clickable=true] > android.view.ViewGroup[id="com.android.systemui:id/system_icons"][clickable=true] > android.widget.LinearLayout[id="com.android.systemui:id/statusIcons"][clickable=true] > android.widget.FrameLayout[desc="WLAN 信号满格。"][id="com.android.systemui:id/wifi_combo"][clickable=true] > android.widget.FrameLayout[id="com.android.systemui:id/wifi_group"][clickable=true] > android.widget.TextView[id="com.android.systemui:id/wifi_standard"][text="6"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 25,
      name: 'in-app|CHKVvROMuM_V0xhl9Qf82o_vL0zOGD79EXi5lID2YNI=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/browser_fragment_layout"][clickable=true] > com.uc.webview.export.WebView[desc="WVUCWebView"][clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[text="幸运包裹"][clickable=true] > android.view.View[id="root"][clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[text="攻略"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 26,
      name: 'in-app|9hi7-DNFyQI0S4qLXLZuhw5dQOI475pVS351P9-TW3Y=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_WN"][clickable=true] > android.view.View[id="_WM"][clickable=true] > android.view.View[id="_WL"][clickable=true] > android.view.View[id="_WK"][clickable=true] > android.view.View[id="_WG"][clickable=true] > android.view.View[id="_Vz"][clickable=true] > android.view.View[id="_Vy"][clickable=true] > android.view.View[id="_Vx"][clickable=true] > android.view.View[id="_Wm"][clickable=true] > android.view.View[id="_Wl"][clickable=true] > android.view.View[id="_Wk"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 27,
      name: 'in-app|EpdHhwtdvYM9x-dHVtzxAGjnfMpI4UUju4jhgfLwfhI=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/gg_dialog_base_body"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/gg_dialog_base_button"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 28,
      name: 'in-app|NK1ngmeAoTE6rMvTSfCVL2WQTKtcOgbm_QNwrTyoW40=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BH"][clickable=true] > android.view.View[id="_BG"][clickable=true] > android.view.View[id="_BF"][clickable=true] > android.view.View[id="_BE"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_At"][clickable=true] > android.view.View[id="_As"][clickable=true] > android.view.View[id="_Ar"][clickable=true] > android.view.View[id="_Ba"][clickable=true] > android.view.View[id="_BZ"][clickable=true] > android.view.View[id="_BY"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 29,
      name: 'in-app|47DEQpj8HBSa-_TImW-5JCeuQeRkm5NMpJWZG3hSuFU=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 30,
      name: 'in-app|mAzZBXjTQdFxTy58J7HUxB1Gy1xzN_Bblpj8MlGtntQ=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Aq"][clickable=true] > android.view.View[id="_Ap"][clickable=true] > android.view.View[id="_Bt"][clickable=true] > android.view.View[id="_Cl"][clickable=true] > android.view.View[id="_Ck"][clickable=true] > android.view.View[id="_CT"][clickable=true] > android.view.View[id="_CS"][clickable=true] > android.view.View[id="_CR"][clickable=true] > android.view.View[id="_CQ"][clickable=true] > android.view.View[id="_CP"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 31,
      name: 'in-app|89HHYxDkFhdZqwVRe7L-o7bwwtUutdq2kKcw69NHHVU=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/close_layout"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 32,
      name: 'in-app|-aZz1xaaKsvE3CxlicSpAKfw2ydbk7RQIiP5uJhLwLY=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            '.components.hybrid.windvane.activity.GuoGuoNewWebViewActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/ptr_frame_layout"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/browser_fragment_layout"][clickable=true] > com.uc.webview.export.WebView[desc="WVUCWebView"][clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[text="幸运包裹"][clickable=true] > android.view.View[id="root"][clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true] > android.view.View[clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 33,
      name: 'in-app|GUvJkCX1CLt5sDGqQBIVDRNIf4DrlFR_zssOUGZ0GKs=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Ao"][clickable=true] > android.view.View[id="_An"][clickable=true] > android.view.View[id="_Am"][clickable=true] > android.view.View[id="_BM"][clickable=true] > android.view.View[id="_BL"][clickable=true] > android.view.View[id="_BK"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 34,
      name: 'in-app|lSc2scNUbTn7NuiA-TtOxkQVvBcoy4NBQBFmGBAhqLs=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.ImageView[id="com.cainiao.wireless:id/draw_dialog_iv_close"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 35,
      name: 'in-app|Z665QQfTStKpy9v-4A7x3mlblUlsYa1i8cvDlpjrdGg=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Jq"][clickable=true] > android.view.View[id="_Jp"][clickable=true] > android.view.View[id="_Jo"][clickable=true] > android.view.View[id="_Jn"][clickable=true] > android.view.View[id="_JF"][clickable=true] > android.view.View[id="_JE"][clickable=true] > android.view.View[id="_JD"][clickable=true] > android.view.View[id="_Iz"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 36,
      name: 'system|OTqKpr0xrWIp084M3Vp9I_6R3h3Ng9soGQir9KiuHPM=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.android.packageinstaller.permission.ui.GrantPermissionsActivity',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.lbe.security.miui:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.lbe.security.miui:id/dialog_root_view"][clickable=true] > android.widget.LinearLayout[id="com.lbe.security.miui:id/parentPanel"][clickable=true] > android.widget.LinearLayout[id="com.lbe.security.miui:id/buttonPanel"][clickable=true] > android.widget.Button[text="仅在使用中允许"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 37,
      name: 'in-app|L0zhoQQlBGKpi-FOzcLxDkov8qh23tyjG0tllu-Q_1g=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/base_webview_activity"][clickable=true] > android.widget.LinearLayout[id="com.cainiao.wireless:id/webview_fragment_layout"][clickable=true] > android.widget.ProgressBar[id="com.cainiao.wireless:id/webview_progress_bar"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 38,
      name: 'in-app|1YK3rNOQWCN-v0OxtywLJZVt5hGybhBdAjicGOmLtng=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BH"][clickable=true] > android.view.View[id="_BG"][clickable=true] > android.view.View[id="_BF"][clickable=true] > android.view.View[id="_BE"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_At"][clickable=true] > android.view.View[id="_As"][clickable=true] > android.view.View[id="_Ar"][clickable=true] > android.view.View[id="_Ba"][clickable=true] > android.view.View[id="_BZ"][clickable=true] > android.view.View[id="_BY"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 39,
      name: 'in-app|S-sJD_4HXUZwu188a41Whkdjs6gMXR-KXJRobbt2z1g=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds:
            'com.alipay.mobile.nebulax.integration.mpaas.activity.NebulaActivity$Main',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_NW"][clickable=true] > android.view.View[id="_NV"][clickable=true] > android.view.View[id="_NU"][clickable=true] > android.view.View[id="_NT"][clickable=true] > android.view.View[id="_Mv"][clickable=true] > android.view.View[id="_Mr"][clickable=true] > android.view.View[id="_Mq"][clickable=true] > android.view.View[id="_Mp"][clickable=true] > android.view.View[id="_SL"][clickable=true] > android.view.View[id="_SK"][clickable=true] > android.view.View[id="_SJ"][clickable=true] > android.view.View[text="确定"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 40,
      name: 'in-app|9a0L0R1wH2Tv_5xUicWkwoZRnRCAhHc3TNkc6eWiL2Y=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_Jo"][clickable=true] > android.view.View[id="_Jn"][clickable=true] > android.view.View[id="_Jm"][clickable=true] > android.view.View[id="_Jl"][clickable=true] > android.view.View[id="_Jh"][clickable=true] > android.view.View[id="_Kx"][clickable=true] > android.view.View[id="_Kw"][clickable=true] > android.view.View[id="_Kv"][clickable=true] > android.view.View[id="_Ku"][clickable=true] > android.view.View[id="_Kt"][clickable=true] > android.view.View[id="_Ks"][clickable=true] > android.view.View[id="_Kr"][clickable=true] > android.view.View[id="_Kq"][clickable=true] > android.view.View[id="_rY"][clickable=true] > android.view.View[id="_rX"][text=""][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 41,
      name: 'in-app|sb880rJ_FsIg_kG70H-9Q--yxVNombvGeBFScecHiq4=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/action_bar_root"][clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/container"][clickable=true] > android.view.ViewGroup[id="com.cainiao.wireless:id/coordinator"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/design_bottom_sheet"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
    {
      key: 42,
      name: 'in-app|w3u5454m2FcKiOEr-Je0jB6pLH0x1T36mvLD8xeHBJI=.png',
      matchTime: 10000,
      actionMaximum: 1,
      resetMatch: 'app',
      priorityTime: 10000,
      rules: [
        {
          action: 'clickCenter',
          activityIds: '',
          matches:
            'android.widget.FrameLayout[clickable=true] > android.widget.LinearLayout[clickable=true] > android.widget.FrameLayout[id="android:id/content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/nebulax_root_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/nebulax_wrapper_view"][clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/fragment_container"][clickable=true] > android.widget.RelativeLayout[clickable=true] > android.widget.RelativeLayout[id="com.cainiao.wireless:id/h5_web_content"][clickable=true] > android.widget.FrameLayout[id="com.cainiao.wireless:id/h5_pc_container"][clickable=true] > com.uc.webview.export.WebView[clickable=true] > com.uc.webkit.bb[clickable=true] > android.webkit.WebView[clickable=true] > android.view.View[id="__react-content"][clickable=true] > android.view.View[id="_BC"][clickable=true] > android.view.View[id="_BB"][clickable=true] > android.view.View[id="_BA"][clickable=true] > android.view.View[id="_Az"][clickable=true] > android.view.View[id="_Av"][clickable=true] > android.view.View[id="_Ao"][clickable=true] > android.view.View[id="_An"][clickable=true] > android.view.View[id="_Am"][clickable=true] > android.view.View[id="_BV"][clickable=true] > android.view.View[id="_BU"][clickable=true] > android.view.View[id="_BT"][clickable=true]',
          snapshotUrls: 'https://i.gkd.li/i/13183946',
        },
      ],
    },
  ],
});
